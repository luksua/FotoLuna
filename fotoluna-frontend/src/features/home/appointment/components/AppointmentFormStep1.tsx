...
const AppointmentStep1Validated: React.FC<AppointmentStep1Props> = ({
    ...
    useEffect(() => {
        ...
        try {
            ...
            // 🚫 Si vengo a CONTINUAR una reserva y el draft es de OTRA cita, lo ignoro y lo borro
            if (
                existingAppointmentId &&
                parsed.appointmentId &&
                parsed.appointmentId !== existingAppointmentId
            ) {
            ...
            // ✅ Solo uso el appointmentId guardado si NO vengo con uno por props
            ...
            // si hay basura, mejor limpiar también
            localStorage.removeItem(APPOINTMENT_DRAFT_KEY);
            setInitialResolved(true);
        }
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            try {
                ...
                const currentEventId = watch("eventIdFK");

                // Si el padre manda initialEventId, le damos prioridad sobre lo que venga del draft
                if (initialEventId && String(initialEventId) !== String(currentEventId)) {
