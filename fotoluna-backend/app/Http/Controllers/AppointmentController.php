...
class AppointmentController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            // si NO hay empleado => se comporta como antes (Pending confirmation)
            $status = $employeeId
                ? 'Scheduled'             // cliente eligió fotógrafo
                : 'Pending confirmation'; // mismo comportamiento que tenía tu amiga

            // ✅ Crear la cita
            $appointment = Appointment::create([
                'customerIdFK' => $customerId,
                'eventIdFK' => $request->eventIdFK,
                'employeeIdFK' => $employeeId, // nuevo, puede ser null
                'appointmentDate' => $request->appointmentDate,
                'appointmentTime' => $request->appointmentTime,
                'appointmentTimeEnd' => $request->appointmentTimeEnd ?? null,
                'place' => $request->place ?? null,
                'comment' => $request->comment,
                'appointmentStatus' => $status,   // aquí usamos la variable
            ]);
